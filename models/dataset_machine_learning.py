...
from sklearn.model_selection import train_test_split, cross_val_score, StratifiedKFold, GroupKFold, TimeSeriesSplit
...
def load_and_prepare_data(file_path):
    ...
    X = df[FEATURES]
    y = df['Label']
    groups = df[GROUP_COL] # <--- Carichiamo i gruppi

    # Split con stratificazione (Manteniamo lo split originale includendo i gruppi)
...
def train_xgboost(X_train, y_train, groups_train, weights_dict):
    ...
    def objective(trial):
        ...
        return cross_val_score(clf, X_train, y_train_encoded, cv=cv, groups=groups_train,
                               scoring='f1_macro', params={'sample_weight': sample_weights}).mean()
...
# 3. LIGHTGBM con TimeSeriesSplit
...
if __name__ == "__main__":
    # Caricamento dati con Subject_ID
    ...
    # Passiamo groups_train a XGBoost
    xgb, le_xgb = train_xgboost(X_train, y_train, groups_train, weights_vector)
